    ...
    return (
        <div className={"salarySection"}>
            ...
            <form className={"salaryForm"}>
                ...
                <label>
                    Miesięcznie:
                    <input value={(hourly.length) > 0 ? (parseInt(hourly) * 160) : monthly} type={"text"} name={"monthly"} placeholder={"zł"} onChange={event => setMonthly(event.target.value)}/>
                    <span> zł</span>
                </label>
                ...
                <label>
                    Na godzinę:
                    <input value={(monthly.length) > 0 ? (parseInt(monthly) / 40) : hourly} type={"text"} name={"hourly"} onChange={event => setHourly(event.target.value)} />
                    <span> zł</span>
                </label>
